...
async function main() {
  ...
  console.log("DirectUBQFamarer deployed to: ", DirectUBQFamarer.target);

  // const DirectUBQFamarer = await hre.ethers.deployContract("DirectUBQFarmer", []);
  // await DirectUBQFamarer.waitForDeployment();
  // console.log("DirectUBQFamarer deployed to: ", DirectUBQFamarer.target);


  console.log('initial ETH balance: ' + hre.ethers.formatEther(await hre.ethers.provider.getBalance(deployer.address)));
  console.log('Exchange ETH to USDT');
  let amount = hre.ethers.parseEther("1");
  ...
  console.log('USDT balance: ' + hre.ethers.formatUnits(await USDT.balanceOf(deployer.address), 6));
  ...
  await USDT.approve(DirectUBQFamarer.target, amountUSDT);
  await DirectUBQFamarer.deposit(USDT.target, amountUSDT, 1);
  console.log('USDT balance: ' + hre.ethers.formatUnits(await USDT.balanceOf(deployer.address), 6));
  console.log('UAD3CRVf balance: ', + hre.ethers.formatEther(await UAD3CRVf.balanceOf(deployer.address)));
  console.log('UBQ balance: ', + hre.ethers.formatEther(await UBQ.balanceOf(deployer.address)));
  ...
  await BondingShareV2.setApprovalForAll(DirectUBQFamarer.target, true);
  await DirectUBQFamarer.withdraw(depositIds[0], USDT.target);
  await BondingShareV2.setApprovalForAll(DirectUBQFamarer.target, false);
